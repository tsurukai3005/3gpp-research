...
本セクションは、Section 5 の各サブセクションを理解するために必要な前提知識を体系的に整理する。TS 38.101-1 の本文は「何が定義されているか」を述べるが、「なぜそう設計されたか」「そもそもその概念は何か」については前提知識として省略されていることが多い。以下の解説は、Section 5 の読解を助けるための補足である。

### 1A.1 バンド（Operating Band）と帯域幅（Channel Bandwidth）の違い
...
CA で組み合わせ可能なバンドの組合せを定義。CA の基本概念については [1A.2](#1a2-キャリアアグリゲーションcaとは) を参照。
...
**バンドコンビネーションの読み方**: 表記の詳細と高次・低次の意味については [1A.3](#1a3-バンドコンビネーションの表記と高次低次の意味) を参照。仕様上、高次のコンビネーションをサポートする UE は、構成要素となる全ての低次コンビネーション（Band Pair）もサポートしなければならない（フォールバック規則）。
...
EN-DC（E-UTRA NR Dual Connectivity）および NR-NR DC で組み合わせ可能なバンド構成を定義。EN-DC は TS 38.101-3 で主に扱われるが、NR-NR DC の構成がここに記載される。DC の基本概念については [1A.4](#1a4-デュアルコネクティビティdcとは) を参照。
...
**前提知識**: バンドと帯域幅の違いについては [1A.1](#1a1-バンドoperating-bandと帯域幅channel-bandwidthの違い) を、gNB と UE の帯域幅の関係および BWP の仕組みについては [1A.5](#1a5-ue-のチャネル帯域幅と基地局の帯域幅の関係) を参照。
...
UE が初期セルサーチで SS/PBCH ブロック（同期信号）を探す周波数位置の集合。チャネルラスターよりもはるかに粗い。セルサーチの全体手順とラスターの階層構造については [1A.6](#1a6-ue-のセルサーチとラスターの仕組み) を参照。
